...
.activation-bar-bg { background: var(--surface2); border-radius: 8px; height: 16px; margin: 8px 0; position: relative; overflow: hidden; }
.activation-bar-fill { border-radius: 8px; height: 16px; transition: width 0.5s; }
...
.summary-label { color: var(--muted); font-size: 11px; text-transform: uppercase; letter-spacing: 2px; }
...
def get_motivational():
    frases = [
        ...
        "Tu unico limite eres vos mismo.",
        "Cada rep cuenta. Cada dia importa.",
...
def get_streak():
    if "streak" not in st.session_state:
        st.session_state.streak = random.randint(1, 7)
...
if not st.session_state.onboarding_done:
    ...
    <div style="max-width:600px;margin:80px auto;text-align:center;">
        ...
        <div style="color:#666;font-size:14px;letter-spacing:3px;text-transform:uppercase;margin:12px 0 40px;">
            Tu guardian virtual de entrenamiento
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin:32px 0;">
            ...
            <div style="background:#111;border:1px solid #222;border-radius:12px;padding:24px 16px;">
                ...
                <div style="color:#666;font-size:12px;margin-top:4px;">Analisis de postura en vivo</div>
            </div>
            <div style="background:#111;border:1px solid #222;border-radius:12px;padding:24px 16px;">
                <div style="font-size:32px;margin-bottom:8px;">&#127897;</div>
        ...
        <div style="color:#444;font-size:13px;margin-bottom:32px;">
            Prevencion de lesiones · Neurociencia aplicada · Coaching personalizado
        </div>
    </div>
    """, unsafe_allow_html=True)
...
for key, val in [
    ("bet_step", 0), ("bet_score", 0), ("bet_complete", False),
    ("bet_start_time", None), ("bet_seq", None), ("bet_ops", None), ("bet_pattern", None),
    ("session_bet_score", None), ("session_postura_score", None), ("session_plan_generado", False),
...
<div class="hero">
    ...
    <div class="hero-sub">Tu guardian virtual de entrenamiento &middot; Powered by Gemini</div>
    <br>
    <span class="tag">&#129504; Brain Endurance Training</span>
    <span class="tag">&#128249; Vision AI</span>
    <span class="tag">&#127897; Live Voice</span>
</div>
""", unsafe_allow_html=True)
...
<div class="welcome-bar">
    <div class="welcome-text">{greeting}, <span>Atleta</span> — listo para entrenar?</div>
    <div class="streak-badge">&#128293; {streak} dias seguidos</div>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    with col1:
        ...
        humor = st.selectbox("Nivel de energia", ["Alta", "Normal", "Baja", "Estresado"])
        horas_sueno = st.slider("Horas de sueno", 0, 12, 7)
        ...
        if st.button("Analizar y generar plan"):
            with st.spinner("FitGuard AI analizando..."):
                prompt = f"Sos FitGuard AI, coach de fitness de elite. El usuario tiene energia {humor}, durmio {horas_sueno} horas. Comentario: {estado_libre}. Dame: 1) Analisis de estado, 2) Tipo de entrenamiento recomendado, 3) Micro-tarea cognitiva BET, 4) Frase motivadora. En espanol, amigable y conciso."
    ...
    with col2:
        ...
        st.write("Eleva tu frecuencia cerebral al nivel optimo para entrenar.")
        ...
        # Barra de activacion cerebral
        score_actual = st.session_state.bet_score
        activacion_pct = min(int((score_actual / 9) * 100), 100)
        color = "#00d084" if activacion_pct >= 66 else "#ffaa00" if activacion_pct >= 33 else "#ff4500"
        st.markdown(f'<div class="activation-label">Nivel de activacion cerebral</div><div class="activation-bar-bg"><div class="activation-bar-fill" style="width:{activacion_pct}%;background:{color}"></div></div>', unsafe_allow_html=True)
        ...
        if st.session_state.bet_step == 1:
            ...
            timer_class = "timer-urgent" if remaining <= 10 else ""
            st.markdown('<div class="bet-step">', unsafe_allow_html=True)
            st.markdown('<div class="bet-number">01</div>', unsafe_allow_html=True)
            st.markdown("**Memoria de Trabajo** — Inverte la secuencia")
            st.info(f"Secuencia: **{st.session_state.bet_seq}**")
            st.markdown(f'<div class="timer-display"><div class="timer-value {timer_class}">{remaining}s</div><div class="timer-label">Tiempo restante</div></div>', unsafe_allow_html=True)
            respuesta = st.text_input("Escribi al reves (ej: 1-9-3-7):", key="r1")
        ...
        if st.session_state.bet_step == 2:
            ...
            st.markdown("**Calculo Mental** — Velocidad + precision")
            st.markdown(f'<div class="timer-display"><div class="timer-value {timer_class}">{remaining}s</div><div class="timer-label">Tiempo restante</div></div>', unsafe_allow_html=True)
            correctos = 0
        ...
        if st.session_state.bet_step == 3:
            ...
            st.markdown("**Patron Visual** — Encontra la secuencia correcta")
            patron = st.session_state.bet_pattern
            siguiente = (patron[-1] % 5) + 1
            st.info(f"Patron: **{' > '.join(map(str, patron))} > ?**")
            otras = [x for x in range(1, 7) if x != siguiente]
            random.shuffle(otras)
            opciones = sorted(otras[:3] + [siguiente])
            st.markdown(f'<div class="timer-display"><div class="timer-value {timer_class}">{remaining}s</div><div class="timer-label">Tiempo restante</div></div>', unsafe_allow_html=True)
            resp = st.radio("Cual sigue?", opciones, key="patron_resp", horizontal=True)
        ...
        if st.session_state.bet_step == 4:
            ...
            st.markdown("**Respiracion Neural** — Sincroniza tu cerebro")
            st.info("Inhala 4s > Retene 4s > Exhala 4s — Repeti 3 veces")
            if st.button("Complete la respiracion", key="c4"):
        ...
        if st.session_state.bet_step == 5:
            ...
            nivel = "OPTIMO" if score >= 6 else "ACTIVADO" if score >= 3 else "EN CALENTAMIENTO"
            emoji = "🟢" if score >= 6 else "🟡" if score >= 3 else "🔴"
            st.markdown(f'<div class="hz-display"><div class="hz-value">{hz.split("-")[0]}</div><div class="hz-label">Hz estimados &middot; {nivel} {emoji} &middot; Score: {score}/9</div></div>', unsafe_allow_html=True)
            with st.spinner("Analizando activacion cerebral..."):
                prompt = f"Atleta completo protocolo BET con puntuacion {score}/9. Estado: {nivel}. Frecuencia: {hz}. Evaluacion cognitiva breve y recomendacion de entrenamiento. Espanol, motivador, conciso."
                response = client.models.generate_content(model="gemini-2.5-flash", contents=prompt)
                st.markdown(response.text)
...
with tab2:
    ...
    with col1:
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">Analisis de Postura</div><div class="accent-bar"></div>', unsafe_allow_html=True)
        ejercicio = st.selectbox("Ejercicio", ["Sentadilla", "Peso muerto", "Plancha", "Flexiones", "Estocada", "Otro"])
        st.markdown('<div class="guide-text">Posicionate frente a la camara y captura tu postura durante el ejercicio</div>', unsafe_allow_html=True)
                ...
                with st.spinner("Vision AI analizando forma..."):
                    ...
                    response = client.models.generate_content(
                        ...
                        contents=[{"role": "user", "parts": [
                            {"text": f"Sos FitGuard AI, experto en biomecanica. Analiza postura en {ejercicio}. Dame: 1) Lo que esta bien, 2) Errores y riesgos, 3) Correcciones especificas, 4) Puntuacion /10. Espanol, conciso."},
                            {"inline_data": {"mime_type": "image/jpeg", "data": img_b64}}
                        ]}]
                    )
                    st.success("Analisis completado")
                    st.markdown(response.text)
                    # Guardar score en sesion
                    st.session_state.session_postura_score = ejercicio
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        if st.button("Nueva micro-tarea"):
            tareas = [
                ...
                "Nombra 5 animales con la misma letra que tu nombre",
                "Visualiza tu proximo set perfecto — 30 segundos",
                "Respiracion: inhala 4s, retene 4s, exhala 4s — 3 veces",
                "Mental: 17 x 6 = ?",
                "Recorda los ultimos 5 ejercicios en orden",
                "Deletrea tu apellido al reves",
                "Suma todos los numeros del 1 al 10",
                "Nombra 3 cosas que ves, 2 que escuchas, 1 que sientes",
                "Conta los cuadrados que hay en la habitacion",
            ]
        ...
        st.info("Para activar el coach de voz en tiempo real, ejecuta en tu terminal:\n\npython voice_agent.py")
        st.markdown('</div>', unsafe_allow_html=True)

# ═══════════════════════════════════════════════════════════════════════════════
with tab3:
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Recuperacion Holistica</div><div class="accent-bar"></div>', unsafe_allow_html=True)

    # Resumen de sesion
    if st.session_state.session_bet_score is not None or st.session_state.session_postura_score is not None:
        st.markdown("**Resumen de tu sesion de hoy:**")
        ...
        with col_s3:
            postura_val = st.session_state.session_postura_score if st.session_state.session_postura_score else "Sin analizar"
            st.metric("Ejercicio analizado", postura_val)
        st.markdown('<div class="divider"></div>', unsafe_allow_html=True)

    col1, col2, col3 = st.columns(3)
    with col1:
        duracion = st.slider("Duracion (min)", 0, 120, 45)
    with col2:
    ...
    notas = st.text_area("Como te sentis?", placeholder="Ej: Cansado pero bien...")
    ...
    if st.button("Generar plan de recuperacion"):
        ...
        with st.spinner("Analizando entrenamiento..."):
            prompt = f"FitGuard AI coach de recuperacion. Entreno {duracion} min, intensidad {intensidad}, dolor en {dolor}. Estado: {notas}. {bet_info}. Dame: 1) Analisis, 2) Estiramientos, 3) Nutricion, 4) Descanso, 5) BET para manana. Espanol, conciso."
            response = client.models.generate_content(model="gemini-2.5-flash", contents=prompt)
            st.success("Plan de recuperacion listo")
            st.markdown(response.text)
    st.markdown('</div>', unsafe_allow_html=True)